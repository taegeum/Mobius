...
var fs = require('fs');                                                     //파일을 입출력할 수 있는 모듈을 가져와 fs에 초기화 한다.
var http = require('http');                                                 //웝서버를 처리할 수 있는 모듈을 가져와 http에 초지화 한다.
var express = require('express');                                           //웹 페이지 개발 및 REST API 개발에 최적화된 모듈을 가져와 express에 초기화 한다.
var bodyParser = require('body-parser');                                    //요청 바디를 파싱하여서 req.body 객체로 접근할 수 있도록 도와주는 모듈을 가져와 bodyParser에 초기화 한다.
var morgan = require('morgan');                                             //format object를 생성하는 리소스 format은 미리 정의된 이름의 문자열(아래 참조), 형식 문자열, 또는 로그 항목을 생성하는 함수
var util = require('util');                                                 // 객체를 문자열로 변환하는 module e.g. util.format('%s', 'foo')
var xml2js = require('xml2js');                                             //xml2js  module 사용해서 xml를 처리한다  e.g.var parser = new xml2js.Parser({explicitArray: false});    xml->json
var url = require('url');                                                   // url.parse() URL 문자열이 객체로 변환되어 반환한다
var xmlbuilder = require('xmlbuilder');                                     //XML 컨트롤(control)생성할 수 있는 module
var ip = require('ip');                                                     //이 module사용해서 ip 받을 수 있다
const crypto = require('crypto');                                           //crypto 모듈 사용하여 시생성 및 암호화를 수행할수 있다.
var fileStreamRotator = require('file-stream-rotator');                     //morgan 로그 분할 작업  （log file rotation） file-stream-rotator 모듈을 사용하여 Apache 조합 형식의 모든 요청을 log / 디렉토리의 날짜 당 하나의 로그 파일에 기록하는 간단한 앱.
var merge = require('merge');                                               //여러 객체를 하나에 병합하여 새로운 복제 객체를 선택적으로 생성합니다 
var https = require('https');                                               //https 요청 보내기 
var cbor = require('cbor');                                                 //간결한 Binary Object Representation (CBOR) 데이터 형식 (RFC7049)으로 데이터를 인 코드하고 파싱합니다.
var moment = require('moment');		                                        //moment module을 이용해서 간단한 동작 타임 체크할 수 있다.시작 시간 기록 및 종료 시 시작시간과 현재시간의 차를 리턴(return)해주고 있습니다
...
global.randomIntInc = function (low, high) {                                            // 임의로 수자타입 값을 반환하는 함수
...
fs.existsSync(logDirectory) || fs.mkdirSync(logDirectory);                              // ensure log directory exists  로그 디텍터리가 있는지 확인
...
app.use(morgan('combined', {stream: accessLogStream}));                                 //combined포맷을 사용하고 stream이라는 object를 생성한것을 이용한다
//ts_app.use(morgan('short', {stream: accessLogStream}));

function del_req_resource() {                                                           //delete_request_resource함수에서는 DB의 delete_req query를 부르는 함수
...
function del_expired_resource() {                                                       //del_expired_resource함수는 만료된 정보를 나타냄
...
var cluster = require('cluster');                                                       //클러스터는 물리적으로 함께 저장되는 하나 이상의 테이블의 그룹입니다
var os = require('os');                                                                 //os를 다루기위한 라이브러리를 가져온다.
var cpuCount = os.cpus().length;                                                        //cpu개수를 가져온다.
var worker = [];
const use_clustering = 1;
if (use_clustering) {                                                                   //사용 되고 있는 cluster가 1대 일때
    if (cluster.isMaster) {                                                             //지금 실행된 인스턴스가 마스터 있을때
        cluster.on('death', function (worker) {                                         //동작이 종료시 다잉 메시지 출력
            console.log('worker' + worker.pid + ' died --> start again');
            cluster.fork();                                                             //시스템이 종료되면 자신을 다시부름
        });

        db.connect(usedbhost, 3306, 'root', usedbpass, function (rsc) {                //애플리케이션 포트 지정하고 db를 접속한다.
           if (rsc == '1') {
                cb.create(function (rsp) {
                    ...
                    require('./pxy_mqtt');                                          //pxy_mqtt모듈을 가져오기 ,MQTT Proxy는 유저의 요청을 한 번 필터링해서 MQTT 브로커토 전달한다. 프락시는 인증된 유저인지, 권한을 가진 Topic에 SUB 요청을 하는지 등을 확인해야 한다
                    require('./pxy_coap');
                    require('./pxy_ws');
...
app.post(onem2mParser, function (request, response) {
    ...
    request.on('end', function () {
        ...
        // console.log('------------------------------------');
        app.post({
            url:'https://e0u6y6t5df.execute-api.ap-northeast-2.amazonaws.com/prod/test',
            data:request.body
        },function(error,data){
            if(error) console.log(error)
            else console.log('apigateway response: \n',data)

        })
        // axios.post('https://e0u6y6t5df.execute-api.ap-northeast-2.amazonaws.com/prod/test', request.body)
